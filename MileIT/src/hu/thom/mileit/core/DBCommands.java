...
public class DBCommands implements Serializable {
	...
	public static final String SQL_S_CAR = "SELECT c.car_id, c.manufacturer, c.model, c.manufacture_date, c.color, c.vin, c.plate_number, c.fuel_capacity, c.fuel, c.start_date, c.end_date, c.description, c.friendly_name, c.active FROM cars AS c WHERE c.car_id = ?";
	...
	// Payment methods
	public static final String SQL_S_PAYMENTS = "SELECT pm.pm_id, pm.name, pm.description FROM payment_method AS pm WHERE pm.user_id = ? ORDER BY pm.name ASC";
	...
	// Locations
	public static final String SQL_S_PLACES = "SELECT p.place_id, p.name, p.address, p.longitude, p.latitude FROM places AS p WHERE p.user_id = ? ORDER BY p.name ASC";
