 ...
 * @author graywatson
 */
public class TOTPManager implements Serializable {
	private static final long serialVersionUID = -7274802494743943453L;
	...
	{
		logger.logInfo("IIB", LogMessages.APPLICATION_TOTP_LICENSE);

		StringBuffer sb = new StringBuffer(NUM_DIGITS_OUTPUT);
	...
	/**
	 * Same as {@link #generateCurrentNumber(String)} except at a particular time in
	 * millis. Mostly for testing purposes.
	 */
	...
	/**
	 * Return the QR image url thanks to Google. This can be shown to the user and
	 * scanned by the authenticator program as an easy way to enter the secret.
	 * 
	 * NOTE: this must be URL escaped if it is to be put into a href on a web-page.
	 */
	public String qrImageUrl(String token) {
		StringBuffer sb = new StringBuffer("https://chart.googleapis.com/chart?chs=200x200&cht=qr&chl=200x200&chld=M|0&cht=qr&chl=").append(token);
		return sb.toString();
	}

	/**
	 * Returns the enrollment token to the user as plain text
	 * 
	 * @param keyId  {@link String} the key of the user
	 * @param secret {@link String} the secret generated by the server
	 * @return {@link String} the complete enrollment token
	 */
	public String enrollToken(String keyId, String secret) {
		StringBuffer sb = new StringBuffer("otpauth://totp/").append(keyId).append("?secret=").append(secret);
		return sb.toString();
	}
	 ...
	 * have the dependency just for this decode method. Exposed for testing.
	 */
