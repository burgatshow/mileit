			...
			<div class="col-md-12">
				<c:if test="${status eq -2}">
					<div class="alert alert-dismissible alert-danger mt-4">
						<fmt:message key="car.status.missing" />
					</div>
				</c:if>
			</div>
		</div>
				...
				<div class="col-md-6">
					<div class="form-group">
						<label for="manufacturer"><fmt:message key="cars.form.manufacturer" /><span class="ml-1 badge badge-pill badge-primary"> <fmt:message
									key="form.mandatory" /></span></label> <input name="manufacturer" type="text"
							class="form-control <c:if test="${validationMessages.contains('manufacturer') }">is-invalid</c:if>" id="manufacturer"
							placeholder="<fmt:message key="cars.form.manufacturer" />"
							value="<c:out value="${not empty param.manufacturer ? param.manufacturer : not empty car ? car.manufacturer : ''}" />">
					</div>
				</div>
				<div class="col-md-6">
					<div class="form-group">
				...
				<div class="col-md-6">
					<div class="form-group">
						<label for="fuel"><fmt:message key="cars.form.fuel" /></label> <select id="fuel"
							...
							<option value="1" <c:if test="${not empty car and car.fuel eq '1'}">selected="selected"</c:if>><fmt:message
									key="cars.form.fuel.petrol" /></option>
							<option value="2" <c:if test="${not empty car and car.fuel eq '2'}">selected="selected"</c:if>><fmt:message
									key="cars.form.fuel.diesel" /></option>
							<option value="3" <c:if test="${not empty car and car.fuel eq '3'}">selected="selected"</c:if>><fmt:message
									key="cars.form.fuel.electric" /></option>
							<option value="4" <c:if test="${not empty car and car.fuel eq '4'}">selected="selected"</c:if>><fmt:message key="cars.form.fuel.other" /></option>
						</select>
					</div>
				</div>
			</div>
